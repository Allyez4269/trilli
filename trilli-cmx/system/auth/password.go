// Package auth provides operator password hashing for CMX. It is a focused port
// of the app's Argon2id implementation (system/auth/password.go) with the
// operator password floor raised to 12 characters per SPEC §6.9 (CMX is a
// god-mode tool, so its credential policy is stricter than the app's 8).
package auth
...
// MinPasswordLen is the operator password floor enforced by HashPassword.
// SPEC §6.9: 12-character minimum.
const MinPasswordLen = 12

var (
	// ErrUnsupportedHash is returned for an unrecognized encoded hash.
	ErrUnsupportedHash = errors.New("auth: unsupported password hash format")
