...
export default function OtpInput({
  ...
  function handleChange(i: number, raw: string) {
    ...
    // If multiple chars arrived (autofill/paste into one box), spread them.
    if (d.length > 1) {
    ...
    <div className="flex items-center justify-center gap-2" onPaste={handlePaste}>
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          className={cn(
            // --input/--background are white in this theme, so a visible field
            // border comes from foreground/25 (matches the app's other inputs).
